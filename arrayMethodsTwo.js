...
// CODE HERE
let balls
const divideTwo = numbers.forEach(nums =>  console.log(nums/2))
...
// CODE HERE
divideAgain = numbers.map(nums => nums/2)
...
// CODE HERE
citiesWithI = cities.filter(city => city.endsWith(`i`))
...
const fruitRace = fruits.forEach((element, index) => {
  if (element.color === `red`) {
    console.log(`The fruit at index ${index} is an apple.`)
...
// The fruit with index 2 is neither apple or orange.
// The fruit at index 3 is an apple.
...
// CODE HERE
const riceDetector = foods.filter(obj => obj.tags.includes(`rice`)).map(obj => obj.price).reduce((acc, cur) => acc + cur)

// THE TOTAL
console.log(riceDetector)
